...
try:
    ...
    if response.status_code in [200, 405]:  # 405 es normal para GET en endpoint POST
        ...
        print(f"   ⚠️  Flask respondió con status {response.status_code}")
...
try:
    ...
    if response.status_code == 200:
        ...
        if tunnels.get('tunnels'):
            ...
            print("   ⚠️  Ngrok corriendo pero sin túneles activos")
    else:
        print(f"   ⚠️  Ngrok respondió con status {response.status_code}")
except Exception as e:
...
print("\n3️⃣  Probando webhook localmente...")
try:
    test_data = {
    ...
    if response.status_code == 200:
        ...
        if '<Message>' in response.text:
            ...
            print("   ⚠️  Respuesta sin formato esperado")
    else:
...
print("\n4️⃣  Verificando credenciales de Twilio...")
